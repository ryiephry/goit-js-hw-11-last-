...
async function handleFormSubmit(e) {
    ...
    if (data.total === 0) alert("Sorry, no images found. Please try again.");
    else createMarkup(data.hits); // a number
    // Reset the input value to an empty string after the form is submitted
    if (e.target.className != "load-more-unhidden") e.target[0].value = "";
}
// Function to fetch data from the Pixabay API based on user input and page number
async function fetchData(userInput, page1) {
    try {
        // Make an HTTP GET request using Axios to the Pixabay API
        const response = await (0, _axiosDefault.default).get(`https://pixabay.com/api/?key=38245803-17b2e774beea3b422758604fe&q=${userInput}&image_type=photo&orientation=horizontal&safesearch=true&page=${page1}&per_page=40`);
